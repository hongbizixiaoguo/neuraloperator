#!/usr/bin/env python3
"""
Darcy方程实验：FNO vs AdaptiveFNO (FnoMoE)对比
使用32分辨率的Darcy Flow数据集
"""
...
def create_experiment_folder():
    """创建实验文件夹"""
    results_dir = Path('/root/autodl-tmp/neuraloperator/experiments/darcy_fno_comparison')
...
def create_models(device):
    ...
    # 标准FNO模型 (基于Darcy配置，适用于32分辨率)
    fno_model = FNO(
        n_modes=(12, 12),  # 对32分辨率合适的模式数
        in_channels=1,     # Darcy数据集输入通道数
        out_channels=1,    # Darcy数据集输出通道数
        hidden_channels=32,
        n_layers=4,
...
def train_model(model, model_name, train_loader, test_loaders, device, n_epochs=100):
    """训练单个模型"""
    ...
    for epoch in range(n_epochs):
        ...
        for batch_data in train_loader:
            # Darcy数据集返回字典格式 {'x': inputs, 'y': outputs}
...
def visualize_results(training_results, eval_results, test_loaders, device, results_dir):
    ...
    plt.suptitle('Darcy Flow: FNO vs AdaptiveFNO (FnoMoE) Training Comparison', 
    ...
    for i in range(4):
        ...
        # 真实输出 (压力场)
        im2 = axes[i, 1].imshow(test_outputs[i, 0].cpu().numpy(), cmap='plasma')
        axes[i, 1].set_title(f'Ground Truth {i+1}')
        axes[i, 1].axis('off')
        
        # FNO预测
        fno_pred = predictions['FNO'][i, 0]
        im3 = axes[i, 2].imshow(fno_pred.numpy(), cmap='plasma')
        axes[i, 2].set_title(f'FNO {i+1}')
        axes[i, 2].axis('off')
        
        # AdaptiveFNO预测
        adaptive_pred = predictions['AdaptiveFNO'][i, 0]
        im4 = axes[i, 3].imshow(adaptive_pred.numpy(), cmap='plasma')
        ...
        vmax = max(error_diff.abs().max().item(), 0.001)
    ...
    plt.suptitle('Darcy Flow: Prediction Comparison (32×32)', 
...
def create_results_table(training_results, eval_results):
    ...
    print(f"\n{'='*80}")
    print(f"DARCY FLOW EQUATION BENCHMARKS (32×32 Resolution)")
    print(f"{'='*80}")
...
def save_results_summary(training_results, eval_results, summary_stats, results_dir):
    ...
    summary_lines = []
    summary_lines.append("DARCY FLOW FNO vs AdaptiveFNO (FnoMoE) COMPARISON")
    summary_lines.append("="*60)
    summary_lines.append("")
    
    # 实验配置
    summary_lines.append("Experiment Configuration:")
    summary_lines.append("  Dataset: Darcy Flow (32×32)")
    summary_lines.append("  Models: FNO vs AdaptiveFNO (4 experts)")
    summary_lines.append("  Training: 100 epochs, AdamW optimizer")
    summary_lines.append("  Loss: H1 (Sobolev norm)")
    summary_lines.append("  Learning Rate: 5e-3")
    summary_lines.append("  Scheduler: StepLR (step_size=60, gamma=0.5)")
    summary_lines.append("")
    ...
    if l2_imp > 0 and h1_imp > 0:
        summary_lines.append("  ✅ 自适应频率选择在Darcy方程上有效! AdaptiveFNO在两个误差指标上都优于FNO")
    elif l2_imp > 0 or h1_imp > 0:
        summary_lines.append("  🔶 自适应频率选择在Darcy方程上部分有效，在某些指标上有改进")
    else:
        summary_lines.append("  ❌ 自适应频率选择在Darcy方程的当前设置下无显著改进")
...
def main():
    """主实验函数"""
    print("🌊 Darcy Flow: FNO vs AdaptiveFNO (FnoMoE) Comparison")
    print("="*60)
    ...
    train_loader, test_loaders, data_processor = load_darcy_flow_small(
        data_root=data_root,
        n_train=1000,  # 1000个训练样本
        batch_size=8,
        test_resolutions=[16, 32],  # 测试16和32分辨率
        n_tests=[100, 50],  # 每个分辨率的测试样本数
    ...
    print(f"   Test samples: 100 (16×16), 50 (32×32)")
    ...
    for model_name, model in models.items():
        result = train_model(
            model, model_name, train_loader, test_loaders, device, n_epochs=100
        )
